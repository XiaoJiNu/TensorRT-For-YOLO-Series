...
public:
    ...
    void
    doInference(IExecutionContext &context, float *input, float *output, const int output_size, cv::Size input_shape);
...
float *YOLO::blobFromImage(cv::Mat &img) {
    ...
    float *blob = new float[img.total() * 3];  //  为什么不用vector<>保存？
    int channels = 3;
    int img_h = img.rows;
    int img_w = img.cols;
    for (size_t c = 0; c < channels; c++) {
        // 先固定一个通道，遍历当前通道上的所有元素，并归一化该元素。
        // 第一次取通道b上所有元素进行归一化，然后去g通道，然后取r通道
        for (size_t h = 0; h < img_h; h++) {  // 遍历每一行
            for (size_t w = 0; w < img_w; w++) {  // 遍历每一列
                // 在内存中，c*h*w这个图片数组是按照bgrbgr...bgr这种方式存放，现在在blob数组中变成bbb...ggg...rrr
...
void YOLO::doInference(IExecutionContext &context, float *input, float *output, const int output_size,
                       cv::Size input_shape) {
